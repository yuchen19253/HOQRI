function [U,S,output] = lmlra_hooi_time(T,U0,options)
...
% Check the tensor T.
if ~isnumeric(T)
    error('lmlra_hooi:T', 'lmlra_hooi currently only accepts dense tensors.');
end
tic
...
while ~output.info
    ...
    G = ttm( tensor(T), {U{1}',U{2}',U{3}'}, [1,2,3]);
    output.normG(end+1) = norm(G)^2;
    output.sangle(end+1) = subspace(U{1},U1);
    output.info = abs(output.sangle(end)) <= options.TolFun;
    fval = 1/2*norm(tensor(T) - ttm( G, {U{1},U{2},U{3}}, [1,2,3] ) )^2;
    output.fval(end+1) = fval;
    output.time(end+1) = toc;
    fprintf('%d %f %f\n',output.iterations, toc, sqrt(2*fval));
